...
- **Authored multi-page navigation** - A session still starts from and is controlled by one HTML entry file, but ordinary links inside that artifact can move the review frame to root-contained local `.html`/`.htm` siblings in the same directory tree. Annotations, questions, snapshots, layout issues, fatal failures, drafts, scroll state, and Mermaid whiteboards retain the page they came from across Back, Forward, reload, delayed Send, and server restart. Lavish adds no page switcher or page indicator: the artifact's own navigation stays authoritative. Remote pages, non-HTML files, directories, escaped symlinks, popups, and nested frames never gain review controls.
  The pending queue, composer draft, and layout-issue selection belong to the current authenticated page. **Send**, **Send & End**, and **Select all** never combine pages; pending work on other pages remains stored and reappears when you navigate back. Polling, export, and share remain entry-file operations.
