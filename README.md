...
- **Authored multi-page navigation** - A session still starts from and is controlled by one HTML entry file, but ordinary links inside that artifact can move the review frame to root-contained local `.html`/`.htm` siblings in the same directory tree. Annotations, questions, snapshots, layout issues, fatal failures, drafts, scroll state, and Mermaid whiteboards retain the page they came from across Back, Forward, reload, delayed Send, and server restart. Lavish adds no page switcher or page indicator: the artifact's own navigation stays authoritative. Remote pages, non-HTML files, directories, escaped symlinks, popups, and nested frames never gain review controls.
  The pending queue, composer draft, and layout-issue selection belong to the current authenticated page. **Send**, **Send & End**, and **Select all** never combine pages; pending work on other pages remains stored and reappears when you navigate back. **Send & End** sends nothing and leaves the review open while another page has unsent feedback, a draft, attachments, or selected layout issues. Use **Send to Agent** for the current page and Back/Forward to send the remaining feedback, then **Send & End** once no other page has pending work. Polling, export, and share remain entry-file operations.
  Submitted feedback waits in durable page batches: each poll returns one page's prompts, fatal failures, and corresponding snapshot in FIFO order. Consecutive submissions on the same page may combine, but A → B → A stays in that order across restart. A terminal send reports the session ended only with the last pending batch.
...
- **Export and sharing** - `lavish-axi export` writes `<name>.export.html` from the session entry file by inlining its local assets only, stripping the annotation SDK, and leaving remote CDN/font references as links that still need network access. Export and share remain entry-only even when the reviewer is currently viewing an authored sibling; they do not package a whole site or expose page proofs.
...
- **Live reload** - Lavish watches the session entry HTML file by default and preserves review context per authored page across reloads: the artifact iframe scroll position, an open annotation card's unsent text, and answers to `data-lavish-question` controls (application-owned form state is left alone). Unsent annotation text also survives a full reload of the review page itself. While viewing a sibling, editing that unwatched sibling requires **Reload artifact**; editing the watched entry reloads the currently viewed eligible page. While a queued layout-issue batch is outstanding, closely spaced saves coalesce so one batch of fixes costs one refresh. To also reload on sibling asset changes, add `data-lavish-live-reload-root` to the root element or `<meta name="lavish-live-reload" content="root">`.
