...
- **Image attachments** - Attach reference images (PNG, JPEG, WebP) to an annotation with the card's **Attach image** control - click it, paste, or drop an image onto it; each shows a thumbnail chip with upload, remove, retry, and error states. Acquisition runs in an isolated, editor-served frame so the artifact under review never touches the image bytes.
  ...
  As a browser-side abuse guard, each chrome page allows 30 upload attempts per rolling minute and 256 MiB of attempted image bytes over its lifetime; rejected uploads stay visible on their annotation card for retry or removal.
