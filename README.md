...
- **Image attachments** - Attach reference images (PNG, JPEG, WebP) with an annotation card's **Attach image** control. It opens a browser-chrome-owned picker where you can choose, paste, or drop images and manage thumbnail chips with upload, remove, retry, and error states; the annotation card itself shows name-only chips. Acquisition runs in the picker's isolated, editor-served frame so the artifact under review never touches image bytes or pixels.
  ...
  As a browser-side abuse guard, each chrome page allows 30 upload attempts per rolling minute and 256 MiB of attempted image bytes over its lifetime; rejected uploads stay visible in the picker for retry or removal, and the annotation card stays open with an error notice.
  Attachments are cleaned up by `LAVISH_AXI_ATTACHMENT_TTL_MS` (default 7 days; `0`/`off` disables) but only once no pending prompt still references them; `LAVISH_AXI_MAX_ATTACHMENT_DISK_MB` (default 512 MiB; `0`/`off` disables) caps total attachment disk, evicting oldest unreferenced files as a backstop.
