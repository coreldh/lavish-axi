...
export function createArtifactSdk(
  ...
  // Match Lavish's own close-button icon (.pill-close / .share-close / .whiteboard-close):
  // same path, viewBox, stroke weight, and round caps, so the chip's remove control reads
  // native to the rest of the UI rather than a foreign glyph.
  const REMOVE_ICON =
    '<svg width="12" height="12" viewBox="0 0 10 10" fill="none" aria-hidden="true" focusable="false"><path d="M1 1L9 9M9 1L1 9" stroke="currentColor" stroke-width="1.6" stroke-linecap="round"/></svg>';
