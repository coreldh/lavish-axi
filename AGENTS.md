...
The SDK's annotation card gates queuing on BOTH `hasPending()` (R2.4, an in-flight upload) AND `hasErrors()` (W2, a failed/rejected chip) so neither is silently dropped by `collectReady`/`closeCard` - it keeps the card open with a notice so the user can wait, retry, or explicitly remove; that notice line is shared with the card's neutral keyboard hint, so it renders in the error color and is restored (never left stale) once the condition clears. Removing a chip posts `lavish:removeAttachment` only for an id nothing can still claim - `classifyAttachmentDelete` skips it when a settled sibling shares that deduped id, and **parks** it while ANY upload is in flight: a twin uploading identical bytes dedups onto that very id but carries no id yet, so an id check alone deletes the file out from under it and the twin's send then fails as not-found (W-B). Parked ids are re-decided as uploads settle; parking can only leak a file (the server's refcount and the TTL sweeper reclaim it), never break a live chip. The card's per-prompt count cap is NOT a literal: `createSdkJs` threads the server's `maxPerPrompt` into the SDK so the local guard matches `LAVISH_AXI_MAX_ATTACHMENTS_PER_PROMPT` (W1), and an over-cap pick is surfaced instantly rather than swallowed. The card re-clamps its viewport position after every attachment-row render (W3) so chip rows can't push Queue/Cancel off-frame; the chip list also has a `max-height`/scroll backstop.
